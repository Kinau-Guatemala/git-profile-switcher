...
- `~/.gitconfig`: only to install a single idempotent `[include] path = ~/.gitconfig-switcher` entry if it is missing.
- `~/.gitconfig-switcher`: the managed file the app owns and rewrites when you apply or undo a profile switch.
